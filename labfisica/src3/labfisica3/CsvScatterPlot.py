...
def main() -> None:
    ...
    pos_data = CsvData(df["t"], df["x"], xName="Tiempo [s]", yName="Posición tabla 3", color="b")
    fig, _ = make_plot(pos_data, scatter=False)
    show_plot(fig)
    save_plot(fig, "plots/posicion_tabla3.png")
  
    vel_data = CsvData(df["t"], df["v"], xName="Tiempo [s]", yName="Velocidad tabla 3", color="r")
    fig, _ = make_plot(vel_data, scatter=False)
    show_plot(fig)
    save_plot(fig, "plots/velocidad_tabla3.png")
