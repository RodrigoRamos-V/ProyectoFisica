...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        print("Uso: python src6/labfisica6/CsvScatterPlot6.py <tabla6.csv>")
        sys.exit(2)
    ...
    pos_data = CsvData(
        df["t"], df["x"],
        xName="Tiempo [s]",
        yName="Posición tabla 6",
        color="b"
    )

    fig, _ = make_plot(pos_data, scatter=False)
    show_plot(fig)
    save_plot(fig, "plots6/posicion6.png")

    vel_data = CsvData(
        df["t"], df["v"],
        xName="Tiempo [s]",
        yName="Velocidad tabla 6",
        color="r"
    )

    fig, _ = make_plot(vel_data, scatter=False)
    show_plot(fig)
    save_plot(fig, "plots6/velocidad6.png")
